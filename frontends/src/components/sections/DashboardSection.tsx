...
export function DashboardSection({ onSelect }: DashboardSectionProps) {
    ...
    const totalFollowers = Object.values(data).reduce((sum, p) => sum + p.followers, 0);
    const totalLikes = Object.values(data).reduce((sum, p) => sum + p.likes, 0);
    const totalViews = Object.values(data).reduce((sum, p) => sum + p.views, 0);
    const totalComments = Object.values(data).reduce((sum, p) => sum + p.comments, 0);
    const totalShares = Object.values(data).reduce((sum, p) => sum + p.shares, 0);
    const totalSaves = Object.values(data).reduce((sum, p) => sum + p.saves, 0);
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                <Card>
                    <CardHeader>
                        <CardTitle>پست‌ها بر اساس پلتفرم</CardTitle>
                    </CardHeader>
                    <CardContent>
                        <div className="h-72">
                ...
                <Card>
                    <CardHeader>
                        <CardTitle>ترکیب تعاملات</CardTitle>
                    </CardHeader>
                    <CardContent>
                        <div className="h-72">
            ...
            <Card>
                <CardHeader>
                    <CardTitle>مرور عملکرد پلتفرم‌ها</CardTitle>
                </CardHeader>
                <CardContent>
                            ...
                            <div key={platform.platform} className="p-4 border rounded-lg space-y-3">
                                ...
                                <div className="space-y-2 text-sm">
                                    <div className="flex justify-between">
                                        <span className="text-muted-foreground">دنبال‌کنندگان:</span>
                                        <span className="font-medium">{platform.followers.toLocaleString()}</span>
                                    </div>
                                    <div className="flex justify-between">
                                        <span className="text-muted-foreground">لایک‌ها:</span>
                                        <span className="font-medium">{platform.likes.toLocaleString()}</span>
                                    </div>
                                    <div className="flex justify-between">
                                    ...
                                    <div className="flex justify-between">
                                        <span className="text-muted-foreground">کامنت‌ها:</span>
                                        <span className="font-medium">{platform.comments.toLocaleString()}</span>
                                    </div>
                                    <div className="flex justify-between">
                                        <span className="text-muted-foreground">اشتراک‌گذاری‌ها:</span>
                                        <span className="font-medium">{platform.shares.toLocaleString()}</span>
                                    </div>
                                    <div className="flex justify-between">
                                        <span className="text-muted-foreground">ذخیره‌ها:</span>
                                        <span className="font-medium">{platform.saves.toLocaleString()}</span>
                                    </div>
                                </div>
                            </div>
                        ))}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <Card>
                    <CardHeader>
                        <CardTitle>برترین پست‌ها</CardTitle>
                    </CardHeader>
                    <CardContent>
                        <div className="space-y-4">
                            {[data.instagram, data.youtube, data.facebook, data.twitter].map((postData, index) => (
                                <div key={index} className="flex items-center justify-between p-3 border rounded-lg">
                                    <div className="flex items-center gap-3">
                                        <div className="w-2 h-2 rounded-full bg-blue-500"/>
                                        <div>
                                            <p className="font-medium text-sm">پست نمونه {index + 1}</p>
                                            <p className="text-xs text-muted-foreground">پلتفرم</p>
                                        </div>
                                    </div>
                ...
                <Card>
                    <CardHeader>
                        <CardTitle>اکشن‌های سریع</CardTitle>
                    </CardHeader>
                    <CardContent>
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            >
                                ...
                                <span className="text-sm font-medium">کمپین جدید</span>
                            </button>
