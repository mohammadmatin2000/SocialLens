...
interface SidebarProps {
  selected: string;
  onSelect: (key: string) => void; // state مرکزی
  isOpen?: boolean;
  onClose?: () => void;
}
...
export function Sidebar({ selected, onSelect, isOpen, onClose }: SidebarProps) {
  const handleSelect = (key: string) => {
    onSelect(key); // تغییر state مرکزی
    if (onClose) onClose(); // بستن در حالت موبایل
  };
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          "fixed top-0 left-0 w-64 h-screen bg-card border-r flex flex-col z-50 transition-transform duration-300 ease-in-out",
